...
if fid>0
    ...
    if isempty(l1)
        answer = questdlg('Do you want to use 2010A mxArray definition?');
        if strcmpi(answer, 'yes')
            here = fileparts(mfilename('fullpath'));
            src = [here filesep() 'Internal_mxArray_2010.h'];
            system(['copy ' src ' ' mxArraydefFilename]);
            content = evalc('type(mxArraydefFilename)');
            return;
        else
