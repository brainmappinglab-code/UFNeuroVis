function cp_matching_folders(search_fldpath, new_basepath)
%cp_matching_folders(search_fldpath, new_basepath)
%   copy matching folders keeping the relative path
    ...
    if isempty(b_fld)
        fprintf('no folder matched. nothing was copied.\n');
    else
        for k=1:length(b_fld)
            ...
            for k2=1:length(c_files)
                ...
                copyfile(c_files{k2},new_sub_fpath,'f');
                fprintf('file %s, copied to : %s\n',c_files{k2},new_sub_fpath);
            end
            
            fprintf('folder %s, copied to : %s\n',b_fld{k},new_fldpath);
        end
