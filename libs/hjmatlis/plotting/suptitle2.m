function hout=suptitle2(str,ax_selected1,title_up_shift)
...
if ~exist('title_up_shift','var')
    title_up_shift=max(thePositions(:,4))*0.3;
end
titlexpos=mean(thePositions(:,1))+mean(thePositions(:,3))/2;
titleypos=max(thePositions(:,2))+max(thePositions(:,4)+title_up_shift);
axes('pos',[0 1 1 1],'visible','off','Tag','suptitle');
ht=text(titlexpos,titleypos-1,str);set(ht,'horizontalalignment','center','fontsize',fs);
set(gcf,'nextplot',np);
axes(haold);
if nargout
    hout=ht;
